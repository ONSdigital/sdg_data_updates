...
colnames(protected_area_main_data) [1, 3] <- c("Year","At_Sea")
...
protected_area_main_data <- within(protected_area_main_data,
...
# Calculations were made to generate the “Proportion of marine protected area” series. 
# Proportion of marine protected area is calculated using the following formula given below. 
# 100 * (Extent at sea - million hectares / UK sea area). 

# calculate and round the value

protected_area_main_data['Proportion_of_area_protected']  <- 
round((protected_area_main_data['At_Sea']/protected_area_main_data['UK_marine_area'])*100, 2)
...
protected_area_main_proportion <- protected_area_main_data  %>%
         mutate("Series" = "Proportion of marine protected area",
           "Units" = "Percentage (%)",
            "Unit multiplier" =  "Unit",
            "Observation status" = "Normal value")
# format column name
colnames(protected_area_main_proportion) [4] <- "Value"

# remove unwanted column
protected_area_main_proportion <- 
within(protected_area_main_proportion, rm("UK_marine_area"))

# save data frame having proper column sequence "Year", "Series", "Observation status",  "Unit multiplier", "Units", "Value"
protected_area_main_proportion = protected_area_main_proportion %>%
...
protected_area_main_extent <- protected_area_main_data  %>%
...
# format column name
colnames(protected_area_main_extent) [2] <- "Value"

# remove unwanted column
protected_area_main_extent <- within(protected_area_main_extent, rm("UK_marine_area"))

# save data frame having proper column sequence "Year", "Series", "Observation status",  "Unit multiplier", "Units", "Value"
protected_area_main_extent = protected_area_main_extent %>%
    select("Year", "Series", "Observation status",  "Unit multiplier", "Units", "Value")

# join dataframe protected_area_main_proportion and protected_area_main_extent vertically(protected_area_main_proportion
# followed by protected_area_main_extent) and save them in csv_output

csv_output <- rbind(protected_area_main_proportion, protected_area_main_extent)
