...
```{r, echo = FALSE}
if(expected_number_of_rows_in_output != nrow(csv_data)) {
  message <- paste0("*There are ", nrow(csv_data), ". *This is different to the expected number of rows. *Please investigate*. The difference might be due to a blank value, which will be shown above. Another likely casue is that there is a different number of rows for 'not stated' and 'impalusible' birthweight rows.")
} else {
  message <- paste0("Check passed: there are ", nrow(csv_data), " rows in the csv.")
}
```
`r message`
