...
data1 <- openxlsx::read.xlsx("Input/PWS_2021.xlsx", sheet = "Data for figure 1", colNames = FALSE) %>% 
  mutate(across(where(is.factor), as.character)) %>% 
  mutate(across(where(is.character), toupper))

data2 <- openxlsx::read.xlsx("Input/SAM_CTRY_DEC_2020_UK.csv", sheet = "Data for figure 2", colNames = FALSE) %>% 
  mutate(across(where(is.factor), as.character)) %>% 
  mutate(across(where(is.character), toupper))
...
header_row_woodland <- which(data1$X1 == "YEAR")

woodland_data_with_header <- data1
...
header_row_certified <- which(data2$X1 == "YEAR")

certified_data_with_header <- data2
