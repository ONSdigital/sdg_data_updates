...
old_data <- read.csv("https://sdgdata.gov.uk/sdg-data/en/data/14-2-1.csv")
# old_data <- read.csv("D:/coding_repos/sdg_data_updates/14-2-1/Example_Output/bcp/14-2-1.csv")

date <- Sys.Date()
new_data <- read.csv(paste0(output_folder, "/", date, "_update_14-2-1", ".csv"))
...
saved as `r paste0(date, "_14-2-1", ".csv")` in the 14-2-1 Output folder.  
...
# colnames(old_data)[2] <- "Series"
# colnames(old_data)[4] <- "Unit measure"
# colnames(old_data)[5] <- "Unit multiplier"
# colnames(old_data)[6] <- "Observation status"
...
```
Warning: This is copied from `update_14-2-1.R` and will not dynamically update 
