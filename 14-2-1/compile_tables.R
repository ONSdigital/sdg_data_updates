...
if (test_run == TRUE) { source("example_config.R") } else if (test_run == FALSE) { 
  source("14-2-1_config.R") } else { 
    stop("test_run must be either TRUE or FALSE") }

source("update_14-2-1.R") # does the donkey-work of making the csv - 
...
csv_filename <- paste0(date, "_update_14-2-1.csv")
qa_filename <- paste0(date, "_14-2-1_checks.html") 
...
rmarkdown::render('14-2-1_checks.Rmd',
