...
markdown to read the text inside the back-ticks as code. 
  ...
  - name of the tabs the data are in: **`r tabname_age`** and **`r tabname_la`** (Note: tabname_la is slightly different from 21/22 onwards compared to earlier years, this will display the tabname for the most recent year read) 
...
# Remove NAs (la_data not included prior to 2019/20)
...
plot_type <- joined_data2 %>%
  ...
  geom_point() +
  geom_line(aes(linetype = dataset)) +
  facet_grid(main_method_of_contraception ~ type_of_contraception) +
  theme_bw() +
  ggtitle("Type of contraception")

# Region name
plot_region <- joined_data2 %>%
  ggplot(.,
         aes(x = year,
             y = value,
             colour = dataset)) +
...
joined_filename = "joined_data.csv"
write.csv(joined_data2, paste0(output_folder, "/", joined_filename), row.names = FALSE)

csv_filename = "csv_output.csv"
