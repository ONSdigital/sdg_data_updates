...
`r first_header_row_country_by_sex`, `r first_header_row_area_of_residence`, and
`r first_header_row_birthweight_by_mum_age` respectively.    
...
if(length(unique(years)) == 1 & length(years) > 1) {
...
### Blank values
Some rows in the original dataset have been suppressed, and have therefore been 
removed from the csv. This is most often the case in cross-disaggregtaions for more extreme groups.
...
neither age nor birthweight are "not stated"). If this table is more than 0 rows, 
...
### Number of rows
We expect there to be `r expected_number_of_rows_in_output` rows in the output 
csv (expected number of rows is set in config.R).  
```{r, echo = FALSE}
if(expected_number_of_rows_in_output != nrow(all_csv_data)) {
  message <- paste0("*There are ", nrow(all_csv_data), ". *This is different to the expected number of rows. *Please investigate*. The difference might be due to a blank value, which will be shown above. Another likely casue is that there is a different number of rows for 'not stated' and 'impalusible' birthweight rows.")
} else {
  message <- paste0("Check passed: there are ", nrow(all_csv_data), " rows in the csv.")
}
```
`r message`
...
### Calculations
For birthweight and mother age, number of late neonatal deaths are calculated as
'number of neonatal deaths - number of earlt neonatal deaths.   
...
where the numerator is greater than 3.
...
N.B. there may be occasional mismatches due to differences in rounding.   
  
See tables below for mismatches:
```{r, echo = FALSE}
rate_mismatches_weight <- SDGupdater::count_mismatches(calculations$Rates_Neonatal_check, 
                                                       calculations$Neonatal_rate)

if(rate_mismatches_weight > 0){
  
table1 <- late_neonatal_weight_age %>% 
  filter(Rates_Neonatal_check != Rates_Neonatal_Deaths) %>% 
    distinct(birthweight, mother_age, Rates_Neonatal_check, Rates_Neonatal_Deaths)
...
rate_mismatches_sex <- SDGupdater::count_mismatches(late_neonatal_country_sex$Rates_Neonatal_check, 
                                                       late_neonatal_country_sex$`Rates_Per_1,000_live_births_Childhood_deaths_Neonatal`)

if(rate_mismatches_sex > 0){
  
table2 <- late_neonatal_country_sex %>% 
  filter(Rates_Neonatal_check != `Rates_Per_1,000_live_births_Childhood_deaths_Neonatal`) %>% 
    distinct(country, sex, Rates_Neonatal_check, 
             `Rates_Per_1,000_live_births_Childhood_deaths_Neonatal`)
...
published neonatal rate by more than 0.15:
...
```{r, echo = FALSE}
wide_data <- all_csv_data %>% 
  mutate(`Neonatal period` = ifelse(`Neonatal period` == "", 
                                    "Neonatal", `Neonatal period`)) %>% 
  pivot_wider(names_from = `Neonatal period`,
              values_from = Value) %>% 
  mutate(sum_early_late = `Early neonatal` + `Late neonatal`) %>% 
  mutate(difference = sum_early_late - Neonatal) %>% 
...
For country and sex the rate for late neonatal deaths is calculated as 
'neonatal death rate - early neonatal death rate' where the count of 
late neonatal deaths is greater than 3. There are two reasons we don't use the 
same calculation as is done for birthweight and age of mother:  
1. Early neonatal death rate is not available for birthweight and age of mother    
2. For Northern Ireland the number of live births *doesn't* include non-residents,
...
country_sex_plot <- all_data %>% 
  ...
  ggplot(data = .,
       aes(x = Year,
           y = Value)) +
  geom_point(aes(colour = `Neonatal period`)) +
  geom_line(aes(colour = `Neonatal period`)) +
