...
filter_unused_out <- function(series_value, var1, var2) {
...
dwell_criteria_ur_plot <- filter_unused_out("Dwellings (%)" , 
...
dwell_criteria_subnat_plot <- joined_data %>%
  filter(series == "Dwellings (%)" &
           (urbanisation_sub_category == "" | is.na(urbanisation_sub_category)) &
           (region == "" | is.na(region)) &
           (deprivation_decile  == "" | is.na(deprivation_decile)) &
           (poverty_status == "" | is.na(poverty_status))) %>% 
  plot_disaggs(test) +
  facet_grid(decent_homes_criteria ~ sub_national_area) 

print(dwell_criteria_subnat_plot)

```

```{r, echo = FALSE, fig.width=10}

dwell_criteria_region_plot <- joined_data %>%
  filter(series == "Dwellings (%)" &
           (urbanisation_sub_category == "" | is.na(urbanisation_sub_category)) &
           (sub_national_area == "" | is.na(sub_national_area)) &
           (deprivation_decile  == "" | is.na(deprivation_decile)) &
           (poverty_status == "" | is.na(poverty_status))) %>% 
  ggplot(.,
         aes(x = year,
  ...
  geom_point() +
  geom_line() +
  facet_grid(decent_homes_criteria ~ region) +
  theme_bw() 

suppressMessages(print(dwell_criteria_region_plot))
