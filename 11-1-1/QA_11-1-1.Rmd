...
Differences may be due to revisions, but could also indicate an error in the 
code.  
**If there are differences please check that new data match source**

```{r, , echo = FALSE}

disaggs <- setdiff(names(joined_data),
                   c("units", "unit_multiplier", "observation_status"))
comparison <- joined_data %>% 
  select(all_of(disaggs)) %>% 
  pivot_wider(names_from = dataset,
              values_from = value) %>% 
  mutate(difference = new - live) %>% 
  filter(difference != 0) %>% 
