...
```{r}
y = max(combined_data_no_duplicates_sorted$Year) - min(combined_data_no_duplicates_sorted$Year) + 1
nrow(combined_data_no_duplicates_sorted) == 99*y + 3*(y - 12)
```


update_indicator has removed all duplicate entries. However, it hsas retained duplicate entries when the Value column is different.
This is in case we had two rows denoting the same data entry but with different values. As a further check we check the number of unique combinations of Year, Series, Country, Sex and Cause of death found in the output is equal to the number of rows in the output.

```{r}
nrow(combined_data_no_duplicates_sorted) == nrow(combined_data_no_duplicates_sorted[c("Country", "Year", "Sex", "Cause of death", "Series")])
```
...
combined_data_cleaned <- combined_data_no_duplicates_sorted %>% 
  mutate(Year = as.integer(Year)) %>%
  filter(Year<=2020) %>% 
  mutate(Year = as.character(Year)) %>% 
  mutate(Value = as.numeric(Value))
...
print(paste("The number of rows in the updated data is: ", nrow(combined_data_no_duplicates_sorted)))

print(paste("Checking for NA in combined_data. Total number of NA is: ", sum(is.na(combined_data_no_duplicates_sorted))))
