...
if ("1-a-1" %in% indicators) {
  print(paste0("OECD GNI url (for 1-a-1): ", gni_api))
}

if("15-a-1_15-b-1" %in% indicators) {
  print(paste0("OECD Biodiversity url (for 15-a-1b and 15-b-1b): ", biodiversity_api))
}

if ("17-7-1" %in% indicators) {
  print(paste0("Low carbon survey data (for 17-7-1 only): ", getwd(), "/", input_folder, "/", filename_lowcarbon))
}
...
- 17-7-1 key words for data extraction from source: low carbon, low-carbon, clean technology, environmental technology, green technology, cleantech, renewable energy, wastewater management, wastewater treatment, energy storage, energy distribution, water remediation, bioenergy, solar, climate friendly, geothermal, climate smart, sustainable energy, air pollution, carbon footprint, global emissions, clean energy, offshore wind, wind energy, wave energy, hydropower, sustainable sanitation, nuclear power, biofuel, atmospheric pollution, biogas, bio-energy, photovoltaic, carbon reduction, energy efficient, biomass energy, nanogrid, nano-grid.
...
```{r, echo = FALSE, fig.height = 8, fig.width = 10}
if ("csv_15a1" %in% objects()) {
  if(num_new_series > 1){
  suppressMessages(print(plot_15a1_series_a))
  suppressMessages(print(plot_15a1_pc_of_gdp))
  suppressMessages(print(plot_15a1_revenue_gbp))
  suppressMessages(print(plot_15a1_revenue_usd))
  suppressMessages(print(plot_15a1_pc_of_revenue))
  }
}
```
...
if ("csv_15a1" %in% objects()) {
  ...
  if (num_new_series > 1){
    ...
    comparison_data_15a1 <- joined_data_15a1 %>% 
      # because the £ is sometimes imported weirdly from a csv, make sure it is always the same for the new and live datasets
      mutate(units = ifelse(grepl("GBP", units) == TRUE, "GBP, millions", units)) %>% 
      mutate(units = ifelse(grepl("USD", units) == TRUE, "USD, millions", units))
    
    comparison_15a1 <- comparison_data_15a1 %>% 
      select(year, series, country_income_classification, 
             units, dataset, value) %>% 
      calculate_differences()
    
    create_datatable(comparison_15a1)
  }
}
```
...
```{r}
#TO BE COMPLETED
if ("csv_1771" %in% objects()) {

joined_data_1771 <- join_plot_data("17-7-1") %>% 
  mutate(units = ifelse(is.na(units), unit_measure, units))
...
#----
  ...
  print("17-7-1 data not produced")
}
```
...
if ("csv_17191" %in% objects()) {
  ...
  usd_plot_17191 <- joined_data_17191 %>% 
    ...
    basic_plot(.) +
    facet_wrap(vars(country_income_classification), nrow = 3) +
    ggtitle("17-19-1 by country income classification (USD)") 
} else {
