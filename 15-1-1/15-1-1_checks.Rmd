...
old_data <- read.csv("https://raw.githubusercontent.com/ONSdigital/sdg-data/master/data/indicator_15-1-1.csv") %>% 
  janitor::clean_names()

date <- Sys.Date()
new_data <- janitor::clean_names(csv_formatted) 
pre_calcs_data <- janitor::clean_names(all_data)
...
status_list <- unique(new_data_to_plot$sustainably_managed_status)

test<- new_data_to_plot %>%
...
new_data_to_plot %>%
    ...
    geom_point() +
    geom_line() +
    facet_grid(country ~ sustainably_managed_status) +
    theme_bw() 
...
```{r echo = FALSE}
new_data %>% 
  filter(sustainably_managed_status == "",
         year == 2021) %>% 
