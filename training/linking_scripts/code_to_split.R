...
#---- Import the datasets ------------------------------------------------------
woodland_source_data <- openxlsx::read.xlsx("training/clean_code/Example_Input/PWS_2021.xlsx", 
                                            sheet = "Data for figure 1", 
                                            colNames = FALSE) %>%
  mutate(across(where(is.factor), as.character))%>%
  mutate(across(where(is.character), toupper))

certified_source_data <- openxlsx::read.xlsx("training/clean_code/Example_Input/PWS_2021.xlsx", 
                                             sheet = "Data for figure 2",
                                             colNames = FALSE) %>%
  mutate(across(where(is.factor),as.character))%>%
  mutate(across(where(is.character),toupper))
...
# add required columns, rename disaggregation levels, and filter out values we 
...
write.csv(csv_formatted,
          'training/clean_code/Output/15-1-1_data.csv',row.names=TRUE)