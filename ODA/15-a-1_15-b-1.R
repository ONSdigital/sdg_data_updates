...
if ("biodiversity_data" %in% objects()){
  # we only want to run this once as it is not split into pre and post 2017 datasets
  # (series a in run twice from compile_tables using the pre_or_post_2017_counter 
  # iterator in a for loop)
  if(pre_or_post_2017_counter == 2){
    ...
    rm(biodiversity, series_b)  
  }
  
} 

# if oecd api didn't work you can still output series a, but the csv is built slightly differently

if ("biodiversity_data" %not_in% objects() |
    ("biodiversity_data" %in% objects() & pre_or_post_2017_counter == 1)) { 
  csv <- series_a %>% 
    arrange(Year, Country_income_classification) %>% 
    replace(is.na(.), "") %>% 
    rename(`Country income classification` = Country_income_classification)
  
  rm(headline, gbp_data, series_a)
}
